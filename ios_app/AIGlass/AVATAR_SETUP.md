...
> ⚠️ **VRoid Studio の VRM 1.0 出力は VRMKit でそのまま読めないことがあります**
> （`firstPerson.meshAnnotations` や `expressions.preset.lookUp` 等の欠落で
> `keyNotFound` エラー）。その場合は次のどちらかで解決します:
> 1. 付属スクリプトで欠落フィールドを補う（見た目は変わりません）:
>    ```
>    python3 ios_app/AIGlass/tools/fix_vrm.py ~/Downloads/書き出した.vrm
>    # → ios_app/AIGlass/AIGlass/model.vrm に修正版を書き出し
>    ```
> 2. もしくは VRoid Studio で **VRM 0.0** として書き出す（VRMKit の VRM0
>    デコーダは安定しており、この問題は起きません）。
